...
include 'controllers/Soccer.php';
 $controller = new Soccer();
 $action = filter_input(INPUT_GET, 'action');
 if (!method_exists($controller, $action)) { $controller->index(); }
...
function create_controller($controller_name) {
  ...
  if (!file_exists($controller_filename)) { throw new Exception(); }
  include $controller_filename;
  if (!class_exists($controller_classname)) { throw new Exception(); }
  return new $controller_classname();
}

function call_controller_method($controller, $method_name, $parameters) {
  $reflectionObject = new ReflectionObject($controller);
  if (! $reflectionObject->hasMethod($method_name)) { throw new Exception("L'action n'existe pas."); }
  $reflectionMethod = $reflectionObject->getMethod($method_name);
  if ($reflectionMethod->getNumberOfParameters()!=count($parameters)){ throw new Exception(); }
...
echo 'parameters : '; var_dump($parameters);echo '<br>'; pour afficher les liens des adresss, On l'aura */ 
